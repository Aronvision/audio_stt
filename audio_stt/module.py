...
class Audio_record:
    def __init__(self):
        ...
        self.vad_sec = 3  # 초기 타임아웃을 3초로 설정
    ...
    def _record_start(self):
        ...
        self.recording = True
        self.buffer = []
        no_voice_target_cnt = (self.vad_sec * 1000)  # 타임아웃을 ms로 변환
        no_voice_cnt = 0
        voice_detected = False  # 음성 감지 여부 플래그
        ...
        with self.microphone as source:
            while self.recording:
                ...
                # 음성이 한 번도 감지되지 않았고 타임아웃이 된 경우
                if not voice_detected and no_voice_cnt >= no_voice_target_cnt:
                    self.recording = False
                # 음성이 감지된 후 무음이 지속되는 경우
                elif voice_detected and no_voice_cnt >= 1000:  # 1초
                    self.recording = False
